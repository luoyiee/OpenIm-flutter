...
import 'set_info/set_info_binding.dart';
import 'set_info/set_info_view.dart';
...
class FriendInfoLogic extends GetxController {
    ...
    if (confirm) {
      await LoadingView.singleton.wrap(asyncFunction: () async {
        ...
        await OpenIM.iMManager.conversationManager
            .deleteConversationAndDeleteAllMsg(conversationID: id);
  ...
  void addFriend() {
    if (userInfo.value.userID == OpenIM.iMManager.userID) {
      IMViews.showToast('不能添加自己');
      return;
    }
  ...
  void getFriendInfo() async {
    ...
    final list = await LoadingView.singleton.wrap(
      asyncFunction: () =>
          Apis.getUserFullInfo(userIDList: [userInfo.value.userID ?? ""]),
    );
    ...
    if (null != info) {
      userInfo.update((val) {
        ...
        val?.isBlacklist = info.isBlacklist;
        val?.isFriendship = info.isFriendship;
      });
    }
  }
  ...
  void recommendFriend() async {
    var result = await AppNavigator.startSelectContacts(
      ...
      excludeIDList: [userInfo.value.userID!],
    );
  ...
  void setMute() {
    ...
    AppNavigator.startSetMuteForGroupMember(
      userID: userInfo.value.userID!,
      groupID: groupID!,
    );
  }
  ...
  /// 是当前登录用户的资料页
  bool isMyself() => userInfo.value.userID == OpenIM.iMManager.userID;
  ...
  bool get isAllowSendMsgNotFriend =>
  ...
  bool get showAddFriendButton =>
      !isMyFriend &&
      (!isGroupMemberPage() ||
          isGroupMemberPage() && !notAllowAddGroupMemberFriend.value);

  bool get isMyFriend => userInfo.value.isFriendship == true;

  bool get canViewProfile {
    return isMyFriend ||
        isGroupMemberPage() && notAllowLookGroupMemberProfiles.value == false;
  }
}

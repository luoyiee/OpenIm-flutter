...
import '../widgets/chat/self/chat_file_preview.dart';
...
class IMUtils {
  ...
  static void openFile(Message msg) async {
    var isFrom = msg.sendID != OpenIM.iMManager.userID;
      ...
      if (null != availablePath) {
        ...
        Get.to(() => ChatFilePreview(
              ...
              onDownloadFinished: () async {
                ...
                IMViews.showToast('文件已保存$cachePath');
                if (null != mimeType && mimeType.contains('video')) {
                  openVideo(
                    Message()
                      ..clientMsgID = msg.clientMsgID
    ...
    String? tag,
      ...
      ChatPicturePreview(
        ...
        onDownloadFinished: (url, path) async {
          ...
          IMViews.showToast('图片已保存${result['filePath']}');
        },
      );
  ...
  static Widget _previewVideo(
      ...
      ChatVideoPlayerView(
        ...
        onDownloadFinished: (url, path) async {
          final result = await ImageGallerySaver.saveFile(path);
          IMViews.showToast('视频已保存${result['filePath']}');
        },
      );
